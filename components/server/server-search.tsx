  ...
  return (
    <>
      ...
      >
        ...
        >
          <span className="text-xs">Ctrl+Shift+K</span>
        </kbd>
      </button>
      <CommandDialog open={open} onOpenChange={setOpen}>
        ...
        <CommandInput placeholder="Найти все каналы и участников"/>
        <CommandList>
          <CommandEmpty>
            Нет результатов
          </CommandEmpty>
            ...
            return (
              <CommandGroup key={label} heading={label}>
                {data?.map(({ id, icon, name}) => {
                  return (
                    <CommandItem className="flex items-center" key={id} onSelect={() => onClick({ id, type })}>
                      {icon}
                      <span>{name}</span>
                    </CommandItem>
                  )
